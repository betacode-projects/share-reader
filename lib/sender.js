    ...
    startAjax(GET_DOWNLOAD_LIST_PAGE, {send_secret_token: $.cookie('sender_secret')}).then(data => {
        ...
        for (let i = 0; i < len; i++){
            ...
            if (download_list.indexOf(recv_token) === -1){
                ...
                $('#history-all').append('<li><strong class="font-weight-bold" style="color: lightgreen">[ダウンロード - '+ row_data['file_downloaded'] +']: </strong><span style="font-style: italic;">'+ recv_token +'</span></li>');
                $('#history-download').append('<li><strong class="font-weight-bold" style="color: lightgreen">'+ row_data['file_downloaded'] +' - </strong><span style="font-style: italic;">'+ recv_token +'</span></li>');
...
function onScanSuccess(qr_data) {
    ...
    startAjax(SET_FILE_PAGE, {send_token: $.cookie('sender'), recv_token: qr_data}).then(data => {
        ...
        $('#history-all').append('<li><strong class="font-weight-bold" style="color: orange">[QR読み取り - '+ date_time +']: </strong><span style="font-style: italic;">'+ qr_data +'</span></li>');
        $('#history-reader').append('<li><strong class="font-weight-bold" style="color: orange">'+ date_time +' - </strong><span style="font-style: italic;">'+ qr_data +'</span></li>');
